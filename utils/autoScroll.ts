...
export const loadScrollStateFromStorage = async (): Promise<ScrollState> => {
...
export const startScrolling = (value: number, labelText?: string) => {
...
export const stopScrolling = async (type?: "pause" | "stop" | "endofpage") => {
  if (scrollInterval) {
    clearInterval(scrollInterval);
    scrollInterval = null;
...
export const doPauseStopOrResume = (type: "pause" | "stop") => {
  if (scrollState.isScrolling) {
    prevScrollValue = scrollState.value;
